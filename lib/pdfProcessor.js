...
var PDFProcessor = {
    ...
    /**
     * Resolve how many pages to read from a given PDF based on prefs.
     *
     *   pageRange.short        (default 2)  – pages to read for short docs
     *   pageRange.long         (default 4)  – pages to read for long docs
     *   pageRange.longThreshold(default 50) – page count above which a doc is "long"
     *
     * Long PDFs (theses, books) commonly have the colophon / publication
     * details on page 3 or 4, so a wider default for them is worth the cost.
     */
    _resolvePagesToRead(pageCount) {
        var shortN = this._getPref("extensions.metadata-filler.pageRange.short", 2);
        var longN = this._getPref("extensions.metadata-filler.pageRange.long", 4);
        var threshold = this._getPref("extensions.metadata-filler.pageRange.longThreshold", 50);
        var n = pageCount > threshold ? longN : shortN;
        return Math.max(1, Math.min(n, pageCount));
    },

    _getPref(key, fallback) {
        try {
            var v = Zotero.Prefs.get(key);
     ...
     *  1. Extract text from N pages (configurable, adaptive to length)
     *  2. Render those pages as images
     *  3. Stitch images side-by-side into one PNG
     *  4. Read embedded PDF metadata (info dict / XMP) when available
     *
     * @param {Zotero.Item} attachment – A PDF attachment item
     * @returns {Promise<{text: string, imageBase64: string|null, pageCount: number, embedded: object|null}>}
     */
    ...
    async _processWithPDFJS(pdfjsLib, data) {
        ...
        const pagesToRead = this._resolvePagesToRead(pageCount);

        // ── Embedded metadata (PDF info dict / XMP) ──
        // These fields are sometimes more reliable than what we'd extract from
        // page imagery — especially for publisher PDFs — so capture them and
        // hand them to the caller for use as a soft prior.
        var embedded = null;
        try {
            ...
            if (meta) {
                ...
                for (var k in embedded) {
                    if (!embedded[k]) delete embedded[k];
                }
                if (!Object.keys(embedded).length) embedded = null;
            }
        } catch (e) {
            Zotero.debug("[MetadataFiller] pdf.getMetadata failed: " + e.message);
        }
        ...
        return { text: fullText.trim(), imageBase64, pageCount, embedded: embedded };
    },
    ...
    async _processTextOnly(attachment) {
        ...
        return { text: text, imageBase64: null, pageCount: 0, embedded: null };
    },
};
