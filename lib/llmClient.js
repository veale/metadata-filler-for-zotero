...
var LLMClient = {
    ...
    async query(opts) {
        ...
        // Custom (OpenAI-compatible) endpoints like Ollama also often run
        // without a key, so we allow that through too.
        if (!apiKey && provider !== "apple" && provider !== "custom") throw new Error(`No API key configured for ${provider}`);
...
7. Do NOT include any explanation, commentary, or markdown formatting. Return ONLY the JSON object.`,
    ...
    async buildAppleHelper(onProgress) {
        ...
        // Resolve / extract the helper source. Tries three paths in order:
        //   1. Unpacked addon directory (rare in Zotero 7 but trivial when present)
        //   2. Already-extracted source under <data dir>/metadata-filler-fm-helper-src/
        //   3. Lazy extraction from the addon (works for packed .xpi installs)
        report("Locating fm-helper source files…");
        var srcDir = await this._ensureFmHelperSrcExtracted(report);


        // 1. Find a real `swift` binary. We deliberately do NOT trust
        // /usr/bin/swift — it exists as a stub even when CLI tools aren't
        // installed, and invoking it triggers a GUI install prompt rather
        // than giving us a clean error. Instead, look for swift at the
        // canonical install locations directly via IOUtils.exists.
        report("Looking for the Swift compiler…");
        var swiftPath = null;
        var swiftCandidates = [
            // Xcode Command Line Tools (free, what `xcode-select --install` gets you)
            "/Library/Developer/CommandLineTools/usr/bin/swift",
            // Full Xcode app (if user has it)
            "/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/swift",
            // Homebrew swift (rare, but cover it)
            "/opt/homebrew/bin/swift",
        ];
        for (var ci = 0; ci < swiftCandidates.length; ci++) {
            try {
                if (await IOUtils.exists(swiftCandidates[ci])) { swiftPath = swiftCandidates[ci]; break; }
            } catch (e) {}
        }
        if (!swiftPath) {
            throw new Error(
                "Could not find a Swift compiler. Looked at:\n"
                + swiftCandidates.map(function(p){ return "  • " + p; }).join("\n")
                + "\n\n"
                + "If you're sure Xcode Command Line Tools are installed, run\n"
                + "  xcode-select -p\n"
                + "in Terminal and paste the output below. Otherwise install the CLI tools:\n"
                + "  xcode-select --install\n"
                + "(macOS pops up a ~3 GB installer; restart this Build helper now button after it finishes.)"
            );
        }
        report("Found swift at: " + swiftPath);

        // 2. swift build. Route through _execExitCode so we handle all
        // Zotero variants (direct number, {exitCode}, undefined) uniformly.
        report("Compiling Swift helper (this takes ~30-60 seconds the first time)…");
        var buildExit;
        try {
            // Use `swift build --package-path <dir>` to avoid needing cwd control.
            buildExit = await this._execExitCode(swiftPath, [
                "build",
                "--package-path", srcDir,
                "-c", "release",
                "--arch", "arm64",
        ...
        // Route through _execExitCode so nsIProcess fallback applies here too.
        report("Ad-hoc signing and marking executable…");
        try { await this._execExitCode("/usr/bin/codesign", ["--sign", "-", "--force", "--timestamp=none", dest]); } catch (e) {}
        try { await this._execExitCode("/bin/chmod", ["+x", dest]); } catch (e) {}
        try { await this._execExitCode("/usr/bin/xattr", ["-d", "com.apple.quarantine", dest]); } catch (e) {}
    ...
    /**
     * Ensure fm-helper/{Package.swift, Sources/fm-helper/main.swift} exist
     * in a writable directory we can point `swift build` at. Tries:
     *
     *   1. Unpacked addon: if `_addonRootPath/fm-helper/Package.swift` exists,
     *      use it directly (no extraction needed).
     *   2. Already-extracted: if files are already at <dataDir>/metadata-filler-fm-helper-src/,
     *      reuse them.
     *   3. Lazy extraction: fetch the two source files from the addon's
     *      rootURI (works for both file:// and jar:file://) and write them
     *      to <dataDir>/metadata-filler-fm-helper-src/. We try fetch() first
     *      and fall back to a synchronous XHR if fetch on jar:file: fails.
     *
     * Returns the path of the directory containing Package.swift on
     * success; throws a detailed error on any failure.
     */
    async _ensureFmHelperSrcExtracted(onProgress) {
        var report = onProgress || function() {};

        // Path 1 — unpacked addon
        if (this._addonRootPath) {
        ...
        var destRoot = PathUtils.join(Zotero.DataDirectory.dir, "metadata-filler-fm-helper-src");
        var sourcesDir = PathUtils.join(destRoot, "Sources", "fm-helper");
        var pkgPath = PathUtils.join(destRoot, "Package.swift");
        var mainPath = PathUtils.join(sourcesDir, "main.swift");

        // Path 2 — already extracted
        try {
            if ((await IOUtils.exists(pkgPath)) && (await IOUtils.exists(mainPath))) {
                this._fmHelperSrcDir = destRoot;
                return destRoot;
            }
        } catch (e) {}

        // Path 3 — extract now
        if (!this._addonRootURI) {
            throw new Error(
                "Could not locate the addon's rootURI to extract fm-helper source from.\n"
                + "Restart Zotero (the rootURI is set during plugin startup) and try again."
            );
        }
        ...
        var files = [
            { url: this._addonRootURI + "fm-helper/Package.swift",                 dest: pkgPath },
            { url: this._addonRootURI + "fm-helper/Sources/fm-helper/main.swift",  dest: mainPath },
        ];

        for (var i = 0; i < files.length; i++) {
            report("Extracting " + files[i].url.split("/").pop() + "…");
            var text = null;
            var fetchErr = null;
            ...
            // Fallback: XMLHttpRequest. Some Zotero builds reject jar:file://
            // fetches with a CORS-ish error but allow XHR through.
            if (text === null) {
                try {
                    text = await new Promise(function(resolve, reject) {
                        var xhr = new XMLHttpRequest();
                        xhr.open("GET", files[i].url, true);
                    ...
                    throw new Error(
                        "Could not read fm-helper source from the addon.\n"
                        + "  URL:        " + files[i].url + "\n"
                        + "  fetch error: " + (fetchErr || "(none)") + "\n"
                        + "  XHR error:   " + (e2 && e2.message || e2) + "\n\n"
                        + "Most likely cause: this .xpi was built without the fm-helper/ source\n"
                        + "(only metadata-filler 1.3.0+ ships it). Update to the latest release.\n\n"
                        + "Manual workaround: clone the repo and build from Terminal:\n"
        ...
        this._fmHelperSrcDir = destRoot;
        report("✓ Source extracted to " + destRoot);
        return destRoot;
    },
    ...
    async _runAppleHelper(helperPath, args) {
        return await this._execExitCode(helperPath, args);
    },

    /**
     * Robust subprocess exec returning the exit code as a number.
     *
     * Tries Zotero.Utilities.Internal.exec first (preferred — handles all
     * the lifecycle cleanup itself), falls back to a hand-rolled nsIProcess
     * wrapper for builds where the helper either isn't available or
     * doesn't resolve to a number. Throws if the binary can't be launched.
     */
    async _execExitCode(binaryPath, args) {
        var ZUI = Zotero.Utilities && Zotero.Utilities.Internal;
        if (ZUI && typeof ZUI.exec === "function") {
            try {
                var ret = await ZUI.exec(binaryPath, args);
                // Some Zotero versions return the exit code directly (number),
                // others return an object like { exitCode }. Normalise.
                if (typeof ret === "number") return ret;
                if (ret && typeof ret === "object" && typeof ret.exitCode === "number") return ret.exitCode;
                if (ret === undefined || ret === null) return 0; // assume success when nothing returned
                Zotero.debug("[MetadataFiller] Internal.exec returned unexpected shape: " + JSON.stringify(ret));
                return 0;
            } catch (e) {
                Zotero.debug("[MetadataFiller] Internal.exec failed, falling back to nsIProcess: " + e.message);
            ...
            try {
                ...
                proc.runAsync(args, args.length, {
                    observe: function(_subject, topic) {
                        ...
                        else if (topic === "process-failed") reject(new Error("process-failed launching " + binaryPath));
                    },
                });
            } catch (e) { reject(e); }
        });
    ...
    // ── Image / multimodal gating ─────────────────────────────

    /**
     * Best-effort check of whether the chosen model can actually read an
     * image. Sending image parts to a text-only model is wasteful at best
     * (extra tokens, higher cost) and a hard 400 error at worst, so we gate
     * on this instead of blindly attaching the image whenever it exists.
     *
     * - Apple's on-device model is text-only (as of macOS 26): always false.
     * - Custom / OpenAI-compatible endpoints (Ollama, LocalAI, …) almost
     *   always run text-only models, so we require a positive vision marker
     *   in the model name (llava, *-vl, pixtral, gemma3, *4o, gemini, …).
     * - Cloud providers default to vision-capable flagships, so we send
     *   unless the name is an obviously text-only / non-chat family.
     *
     * Name heuristics are imperfect; the user can always override by setting
     * a custom model name that matches (or doesn't) these patterns.
     */
    _modelSupportsVision(provider, model) {
        if (provider === "apple") return false;
        var m = String(model || "").toLowerCase();

        if (provider === "custom") {
            return /vision|vl\b|-vl|llava|bakllava|moondream|minicpm-?v|pixtral|gemma-?3|multimodal|4o|4\.1|gemini|claude-[345]|claude-(sonnet|opus|haiku)|llama-?3\.2-vision/.test(m);
        }

        // Cloud providers (openai / anthropic / google): vision-capable by
        // default, minus the clearly text-only / non-multimodal families.
        if (/gpt-3\.5|text-|embedding|whisper|tts|davinci|babbage|moderation/.test(m)) return false;
        return true;
    },

    /**
     * Decide whether to attach the rendered page image for this call, and
     * return a short human-readable reason for the dialog log. Centralises
     * the opt-in + vision-capability + availability logic so the manual scan
     * and Quick Fill flows behave (and report) identically.
     *
     * @returns {{send: boolean, reason: string}}
     */
    resolveImageDecision(provider, model, sendImages, hasImage) {
        if (!sendImages) {
            return { send: false, reason: hasImage ? "available (images off — opt in to send)" : "no" };
        }
        if (!hasImage) {
            return { send: false, reason: "no (none rendered — text-only)" };
        }
        if (!this._modelSupportsVision(provider, model)) {
            return { send: false, reason: "available (skipped — " + (model || provider) + " is not vision-capable)" };
        }
        return { send: true, reason: "yes (sending)" };
    },
