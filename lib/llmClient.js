...
7. Do NOT include any explanation, commentary, or markdown formatting. Return ONLY the JSON object.`,
    ...
    async buildAppleHelper(onProgress) {
        ...
        if (!this._addonRootPath) {
            throw new Error("Could not locate the addon's source directory. Reinstall the plugin from a recent .xpi.");
        }

        var srcDir = PathUtils.join(this._addonRootPath, "fm-helper");
        if (!(await IOUtils.exists(srcDir))) {
            throw new Error(
                "fm-helper/ source not found inside the addon at:\n  " + srcDir + "\n\n"
                + "This .xpi was built without the helper source. Either:\n"
                + "  • Install a more recent .xpi (built from the repo's main branch); or\n"
                + "  • Clone the repo and run 'cd fm-helper && swift build -c release --arch arm64' manually."
            );
        }
